...
(function ($) {
    ...
    $("#example1").DataTable({
        ...
        dom:
            "<'row'<'col-sm-4'l><'col-sm-4'><'col-sm-4'f>>" +
            "<'row'<'col-sm-12'tr>>" +
    ...
    $(document).on(
        "submit",
        "form:not(.modal form,.chat-system form)",
        function () {
            if ($(this).valid()) {
                $(".preloader").show();
                $(".loader").show();
    ...
    $.ajax({
        ...
        success: function (visits) {
            ...
            if (visits.length > 0) {
                ...
                $(".list_visits").html(
                    `<p class="text-center">` + trans("No new visits") + `</p>`
                );
            }
        },
    });
    ...
    $.ajax({
        ...
        success: function (stock_alerts) {
            ...
            if (stock_alerts.length > 0) {
                html +=
                    `<a href="` +
                    url("admin/inventory/product_alerts") +
                    `" class="dropdown-item">
                  <i class="fas fa-cubes mr-2"></i> <span class="badge badge-danger">` +
                    stock_alerts.length +
                    `</span> ` +
                ...
                $(".list_stock_alerts").html(
                    `<p class="text-center">` +
                        trans("No stock alerts") +
                        `</p>`
                );
            }
        },
