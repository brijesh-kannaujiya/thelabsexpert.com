<td data-select2-id="44">
    <span id="bookingStatus-716294">
        ...
        <div class="btn-group dropright">
            <button type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" title="View History" class="btn btn-sm btn-outline-dark dropdown-toggle">
            </button>
            <div class="dropdown-menu px-2 py-2" style="width: 20rem;">
                <ul class="list-group list-group-flush">
                    @foreach ($booking->status as $status)
                    <li class="list-group-item">{{$status->name}}: <span class="st_time_716294_on_the_way"> </span>{{$status->pivot->created_at}}</li>
                    @endforeach
                </ul>
        ...
        {{-- @json($booking->status) --}}
        <hr><span class="st_time"> @if ($booking->status->isNotEmpty())
    ...
    <br>
    <div class="btn-group dropleft mr-2 mb-2 "><button type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" title="View Vials" class="btn btn-sm btn-outline-dark dropdown-toggle">
            View Vials </button>
